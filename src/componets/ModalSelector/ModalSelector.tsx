...
function ModalSelector({ items, value, onChange }: Props) {
  ...
  return (
    <div>
      <button
        onClick={onOpen}
        className="bg-default-100 w-full px-2 py-3 rounded-lg flex items-center"
      >
        <span
          className={classNames(
            "w-8 h-8 rounded-full text-white mr-2 flex justify-center items-center",
            selectedItem.iconColor ? selectedItem.iconColor : "bg-green-700"
          )}
        >
              ...
              {filteredItems.map((item) => (
                <li key={item.name} className="mb-2">
                  ...
                  >
                    <span
                      className={classNames(
                        "w-8 h-8 rounded-full mr-4 flex justify-center items-center text-white",
                        item.iconColor ? item.iconColor : "bg-green-700"
                      )}
                    >
